		...
		for(j in 1:length(nc_lon))	{
			...
			if(any(!is.na(nc_dummy) & any(nc_dummy != missing_data)))	{
				...
				for(thisYear in truncYears)	{
					...
					gfdl_cv =   c(gfdl_cv,   sd(gfdl_yrly[theseYears])   / mean(gfdl_yrly[theseYears]))
					hadgem_cv = c(hadgem_cv, sd(hadgem_yrly[theseYears]) / mean(hadgem_yrly[theseYears]))
					ipsl_cv =   c(ipsl_cv,   sd(ipsl_yrly[theseYears])   / mean(ipsl_yrly[theseYears]))
					miroc_cv =  c(miroc_cv,  sd(miroc_yrly[theseYears])  / mean(miroc_yrly[theseYears]))
				}
...
#	saveRDS(dataOutArray, file=paste0(ncpath, 'data_out.rds')) # rcp 2.6
	saveRDS(dataOutArray, file=paste0(ncpath, 'data_out2.rds'))# rcp 6.0
	}
	nc_close(ncin_gfdl)
	nc_close(ncin_hadgem)
	nc_close(ncin_ipsl)
...
dataOutArray = readRDS(file=paste0(ncpath, 'data_out2.rds'))	# rcp 6.0
...
metadata = list(tcfdVariable = list(units = 'Surface Water Interannual Variability (CV)'))
...
names(dim(valueClass)) = 'valueClass'
	...
	# saving ncdf
ArrayToNc(list(tcfdVariable, lon, lat, decade, rcpScen, valueClass), file_path = paste0(ncOutputPath, 'WRI_basedStreamflowInterannualVariability_v2_processed.nc'))

	# testing output, squinty eye test
myNC = nc_open(paste0(ncOutputPath, 'WRI_basedStreamflowInterannualVariability_v2_processed.nc'))
