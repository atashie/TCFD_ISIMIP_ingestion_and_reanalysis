...
climateDataPlotting_f = function(	
	...
	for(thisLoc in 1:nrow(customerTable_input))	{
		for(thisScen in 1:length(scenarioNames))	{
			...
			png(paste0(customerFolder_input, clientName_input, '\\',  customerTable_input[thisLoc, ..locationHeader], '_', scenarioNames[thisScen], "_rawValues.png"), width=900, height=900)
...
###################################################################
# 4A- incorporating 3rd party data for historicals
...
gracePlotter_f = function(
	...
	#	boxWeighting = 1 - distanceBox # since max distance of a 2x2 .5 deg grid is 1 deg
		...
		for(thisWeight in 2:length(boxWeighting))	{
			...
			# water index deficits
		png(paste0(customerFolder_input, clientName_input, '\\',  customerTable_input[thisLoc, ..locationHeader], '_', "_GRACE-historicalGW.png"), width=900, height=600)
	...
	#		col='#4cbfad', lwd=3) #015f6f
		...
		graceDataTable = rbind(graceDataTable, data.table(
			Location = unlist(customerTable_input[thisLoc, ..locationHeader]),
