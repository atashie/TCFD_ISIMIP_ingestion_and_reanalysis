...
for(i in scenarios)	{
	...
	nc_testDat = ncvar_get(nc_gfdl_init, ncVarFileName2)
	...
	nc_testDat = ncvar_get(nc_hadgem_init, ncVarFileName2)
	...
	nc_testDat = ncvar_get(nc_ipsl_init, ncVarFileName2)
	image(nc_lon, rev(nc_lat), log(nc_testDat[,,9]))
	...
	nc_testDat = ncvar_get(nc_miroc_init, ncVarFileName2)
	image(nc_lon, rev(nc_lat), log(nc_testDat[,,9]))
...
initDates = 1:168
ncin_gfdl = nc_open(paste0(ncpath, 'baselineWaterDepletion_gfdl26.nc'))
nc_gfdl_init = ncvar_get(ncin_gfdl, ncVarFileName2)[ , , initDates]	# lon, lat, time
...
ncin_hadgem = nc_open(paste0(ncpath, 'baselineWaterDepletion_hadgem26.nc'))
nc_hadgem_init = ncvar_get(ncin_hadgem, ncVarFileName2)[ , , initDates]	# lon, lat, time
nc_close(ncin_hadgem)

ncin_ipsl = nc_open(paste0(ncpath, 'baselineWaterDepletion_ipsl26.nc'))
nc_ipsl_init = ncvar_get(ncin_ipsl, ncVarFileName2)[ , , initDates]	# lon, lat, time
nc_close(ncin_ipsl)

ncin_miroc = nc_open(paste0(ncpath, 'baselineWaterDepletion_miroc26.nc'))
nc_miroc_init = ncvar_get(ncin_miroc, ncVarFileName2)[ , , initDates]	# lon, lat, time
nc_close(ncin_miroc)
...
metadata = list(tcfdVariable = list(units = 'Water Depletion [ - ]'))
...
names(dim(valueClass)) = 'valueClass'
	...
	# saving ncdf
ArrayToNc(list(tcfdVariable, lon, lat, decade, rcpScen, valueClass), file_path = paste0(ncOutputPath,'WRI_based_WaterDepletion_v2_processed.nc'))

	# testing output, squinty eye test
myNC = nc_open(paste0(ncOutputPath, 'WRI_based_WaterDepletion_v2_processed.nc'))
