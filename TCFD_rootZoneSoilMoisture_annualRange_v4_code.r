		...
		for(j in 1:length(nc_lon))	{
			...
			if(any(!is.na(nc_dummy) & any(nc_dummy != missing_data)))	{
				...
				for(kh in 1:(length(nc_1019)/12))	{
					data1019 = c(data1019,
						diff(range(nc_1019[(1:12)+(12*(kh-1))])))
				}
				data2029 = NULL
				for(kh in 1:(length(nc_2029)/12))	{
					data2029 = c(data2029,
						diff(range(nc_2029[(1:12)+(12*(kh-1))])))
				}
				data3039 = NULL
				for(kh in 1:(length(nc_3039)/12))	{
					data3039 = c(data3039,
						diff(range(nc_3039[(1:12)+(12*(kh-1))])))
				}
				data4049 = NULL
				for(kh in 1:(length(nc_4049)/12))	{
					data4049 = c(data4049,
						diff(range(nc_4049[(1:12)+(12*(kh-1))])))
				}
				data5059 = NULL
				for(kh in 1:(length(nc_5059)/12))	{
					data5059 = c(data5059,
						diff(range(nc_5059[(1:12)+(12*(kh-1))])))
				}
				data6069 = NULL
				for(kh in 1:(length(nc_6069)/12))	{
					data6069 = c(data6069,
						diff(range(nc_6069[(1:12)+(12*(kh-1))])))
				}
				data7079 = NULL
				for(kh in 1:(length(nc_7079)/12))	{
					data7079 = c(data7079,
						diff(range(nc_7079[(1:12)+(12*(kh-1))])))
				}
				data8089 = NULL
				for(kh in 1:(length(nc_8089)/12))	{
					data8089 = c(data8089,
						diff(range(nc_8089[(1:12)+(12*(kh-1))])))
				}
				data9099 = NULL
				for(kh in 1:(length(nc_9099)/12))	{
					data9099 = c(data9099,
						diff(range(nc_9099[(1:12)+(12*(kh-1))])))
				}
...
metadata = list(tcfdVariable = list(units = 'Root Zone Soil Moisture - annual range in mm'))
...
names(dim(valueClass)) = 'valueClass'
	...
	# saving ncdf
ArrayToNc(list(tcfdVariable, lon, lat, decade, rcpScen, valueClass), file_path = paste0(ncOutputPath, ncVarFileName, 'range_processed.nc'))

	# testing output, squinty eye test
myNC = nc_open(paste0(ncOutputPath, ncVarFileName, 'range_processed.nc'))
