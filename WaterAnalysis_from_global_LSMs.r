...
customerFolder = 'J:\\Cai_data\\Rabo\\'
thisDate = Sys.Date()
...
latlonBox_local = c(-57,-16,-77,-64)#c(35,53,-12,46)
...
myTitle = 'SM (pwp) interannual reg'
...
geomIntersects = st_intersection(df_proj, thisCountry)
...
#	geom_sf(data = geomIntersects, size=6.4, shape=15, aes(fill=plotData, color=plotData),  linewidth = 0.0001) +
	with_blur(geom_raster(aes(y = y, x = x, fill = plotData), interpolate = FALSE), sigma = 8) +
...
ggsave(paste0(customerFolder, varName, "class.png"), width = 4, height = 10)
...
varName = 'aridityIndex_transientSeasonal_ssp_v2_processed'
nc_close(myNC)
myNC = nc_open(paste0(ncOutputPath, varName, '.nc'))
nc_lat = ncvar_get(myNC, 'lat')	# lat is given from high to low
nc_lon = ncvar_get(myNC, 'lon')
