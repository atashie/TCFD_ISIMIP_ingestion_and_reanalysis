...
#	cropPhenology_df = data.table::fread(cropPhenologyTable)
	...
	for(thisRow in 1:nrow(customerTable_input))	{
		...
		pptQntsNrml = 1 - ((climateData[thisRow, , rep(17,8), , 1] - climateData[thisRow, , c(14:20,13), , 1]) /  climateData[thisRow, , rep(17,8), , 1])
		
		pptQntsAvg = sqrt(pptQntsNrml) * climateData[thisRow, , rep(17,8), , 1] # taking the sqrt since the var will be multiplied (and e.g. Q05*Q05 --> Q0025)
		pptQntsDrght = sqrt(pptQntsNrml) * climateData[thisRow, , rep(15,8), , 1]
		pptQntsDrghtShft = sqrt(pptQntsNrml) * (1 - ((climateData[thisRow, , rep(17,8), , 1] - climateData[thisRow, , rep(15,8), , 1]) / climateData[thisRow, , rep(17,8), , 1]))

		#pet = potential evapotranspiration
		petQntsNrml = 1 - ((climateData[thisRow, , rep(17,8), , 2] - climateData[thisRow, , c(20:14,13), , 2]) /  climateData[thisRow, , rep(17,8), , 2])
		petQntsAvg = sqrt(petQntsNrml) * climateData[thisRow, , rep(17,8), , 2]
	#	petQntsDrght = sqrt(petQntsNrml) * climateData[thisRow, , rep(19,7), , 2]
		...
		strmflQntsNrml = 1 - ((climateData[thisRow, , rep(17,8), , 4] - climateData[thisRow, , c(14:20,13), , 4]) /  climateData[thisRow, , rep(17,8), , 4])
